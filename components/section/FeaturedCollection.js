...
const FeaturedCollection = ({ fields }) => {
  const { title, collectionHandle } = fields;
  const [collectionProductData, setCollectionProductData] = useState([]);
  useEffect(() => {
    const variables = {
      filter: { first: 1, locale: 'en-US', handles: [collectionHandle] }
    };
    nacelleClient
      .query({
      ...
      .then(({ productCollections }) => {
        setCollectionProductData(productCollections[0].products);
      });
  }, [collectionHandle]);
  return (
    <section className="bg-white" section="FeaturedProducts">
      <div className="max-w-7xl mx-auto py-16 px-4 sm:py-24 sm:px-6 lg:px-8">
        <div className="sm:flex sm:items-baseline sm:justify-between">
          ...
          >
            Browse our {title.toLowerCase()} <span aria-hidden="true">→</span>
          </a>
        </div>
